$(document).ready(function () {
    ...
    function initCrowdSelectionHandlers() {
        ...
        $('#add-condition-btn').click(function () {
            ...
            $.ajax({
                ...
                type: 'GET',
                success: function (response) {
                    const tagOptions = response.tags.map(tag => `<option value="${tag}">${tag}</option>`).join('');
        ...
        $('#submit-crowd-selection-btn').click(function () {
            ...
            $('#condition-container .condition-row').each(function () {
                ...
                selectedConditions.push({ tag_name: tag, tag_value: value });
            });
            ...
            $.ajax({
                ...
                success: function (response) {
                    $('#result').text('预圈选结果: ' + response.result);
                },
                error: function () {
                    alert('预圈选失败，请稍后重试');
                }
    ...
    function initRuleParametersHandlers() {
        let selectedEventProps = [];  // 用于保存当前事件的props及对应的输入值
        ...
        // 提交规则按钮点击事件
        $('#submit-rule-btn').click(function () {
            const eventId = $('#event-select').val();  // 获取选择的事件 ID
            const startTime = $('#start-time').val();  // 获取起始时间
            const endTime = $('#end-time').val();  // 获取结束时间
            const queryTime = new Date().toISOString();  // 获取当前时间作为查询时间

            // 获取人群圈选表单中的条件
            const queryConditions = [];
            $('#props-container .prop-row').each(function () {
                const tagName = $(this).find('.tag-select').val();  // 获取条件的标签名
                const tagValue = $(this).find('.tag-value').val();  // 获取条件的标签值
                queryConditions.push({ tag_name: tagName, tag_value: tagValue });
            });
            ...
            $('#props-container .prop-row').each(function () {
                ...
                if (propName && propValue) {  // 如果propName和propValue都不为空，则添加到eventProps
                    eventProps.push({ prop_name: propName, prop_value: propValue });
                }
            });
            ...
            const requestData = {
                ...
                event_props: eventProps  // 添加event_props
            };

            // 发送POST请求
            $.ajax({
                url: '/api/dq/check',  // 后端接口地址
                type: 'POST',
                contentType: 'application/json',
                data: JSON.stringify(requestData),
                success: function (response) {
